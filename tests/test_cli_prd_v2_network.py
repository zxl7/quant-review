...
class RunRebuildPrdV2NetworkTest(unittest.TestCase):
    ...
    def test_main_returns_130_when_fetch_is_interrupted(self) -> None:
        with patch.object(cli, "run_fetch_and_rebuild", side_effect=KeyboardInterrupt), patch(
            ...
            rc = cli.main(["--fetch", "--date", "2026-06-23"])

        self.assertEqual(rc, 130)
        self.assertTrue(any("用户中断" in str(call.args[0]) for call in mock_print.call_args_list if call.args))


class RunFetchAndRebuildDailySnapshotLimitTest(unittest.TestCase):
    def test_fast_trade_date_resolver_uses_local_cache_without_online_probe(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(cli, "resolve_trade_date") as mock_resolve:
                actual, note = cli._resolve_trade_date_fast(
                    client=object(),
                    requested_date="2026-06-23",
                    cache_dir=cache_dir,
                )

        self.assertEqual(actual, "2026-06-23")
        self.assertIn("本地交易日缓存命中", note)
        mock_resolve.assert_not_called()

    def test_trade_days_fallback_persists_actual_date_when_online_calendar_fails(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            cache_dir = Path(tmp) / "cache"
            cache_dir.mkdir()
            (cache_dir / "trade_days_cache.json").write_text(
            ...
            http_error = urllib.error.HTTPError(
                url="https://example.test/hsindex/history",
                code=500,
            ...
            with patch.object(cli, "get_trading_days_from_index_k", side_effect=http_error):
                result = cli._resolve_trade_days_with_local_fallback(
                    client=object(),
                    cache_dir=cache_dir,
            ...
            payload = json.loads((cache_dir / "trade_days_cache.json").read_text(encoding="utf-8"))

        self.assertEqual(result[-2:], ["2026-06-22", "2026-06-23"])
        self.assertIn("2026-06-23", payload["days"])
        self.assertEqual(payload["as_of"], "2026-06-23")

    def test_trade_days_resolver_uses_local_sequence_without_online_probe_when_fresh(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            cache_dir = Path(tmp) / "cache"
            cache_dir.mkdir()
            (cache_dir / "trade_days_cache.json").write_text(
            ...
            with patch.object(cli, "get_trading_days_from_index_k") as mock_get_days:
                result = cli._resolve_trade_days_with_local_fallback(
                    client=object(),
                    cache_dir=cache_dir,
                    actual_date="2026-06-23",
        ...
        self.assertEqual(result, ["2026-06-19", "2026-06-22", "2026-06-23"])
    ...
    def test_run_fetch_and_rebuild_uses_short_timeout_client_for_pool_refresh(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            cache_dir = root / "cache"
            cache_dir.mkdir()
            ...
            created_clients: list[tuple[int, int]] = []

            class FakeClient:
                def __init__(self, *, base_url: str, token: str, timeout: int, retries: int = 2):
                    created_clients.append((timeout, retries))
            ...
            with ExitStack() as stack:
                ...
                stack.enter_context(patch("daily_review.http.HttpClient", FakeClient))
                ...
                mock_fetch_pool = stack.enter_context(patch.object(cli, "_fetch_pool_with_cache_fallback", return_value=[]))
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with ExitStack() as stack:
                ...
                stack.enter_context(
                    patch.dict(
                        cli.os.environ,
                        {"QR_LIMIT_DAILY_SNAPSHOT": "1", "QR_ENABLE_CORE_QUOTES_REFRESH": "1"},
                        clear=False,
                    )
                )
                rc = cli.run_fetch_and_rebuild("2026-06-23")
    ...
    def test_run_fetch_and_rebuild_limits_full_publish_quotes_to_core_pool(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with ExitStack() as stack:
                ...
                stack.enter_context(patch.object(cli, "attach_stock_research_backtest"))
                stack.enter_context(patch.object(cli.time, "sleep"))
                stack.enter_context(patch.dict(cli.os.environ, {"QR_ENABLE_CORE_QUOTES_REFRESH": "1"}, clear=False))
                rc = cli.run_fetch_and_rebuild("2026-06-23")
    ...
    def test_run_fetch_and_rebuild_skips_core_quotes_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            cache_dir = root / "cache"
            cache_dir.mkdir()
            ...
            fake_cfg = SimpleNamespace(base_url="https://example.test", token="token")
            raw_pools = {
                "ztgc": [{"dm": "000001", "lbc": 1}],
                "yest_ztgc": [{"dm": "000002", "lbc": 2}],
            }
            logs: list[str] = []
            ...
            with ExitStack() as stack:
                ...
                mock_quotes = stack.enter_context(patch.object(cli, "_fetch_realtime_quotes_map", return_value={"000001": {"dm": "000001"}}))
                mock_attach = stack.enter_context(patch.object(cli, "attach_quotes_and_features"))
                stack.enter_context(patch.object(cli, "write_market_data", side_effect=fake_write_market_data))
                stack.enter_context(patch.object(cli, "run_rebuild", return_value=0))
                stack.enter_context(patch.object(cli, "append_watch_runtime_slice"))
                stack.enter_context(patch.object(cli, "inject_intraday_snapshots"))
        ...
        self.assertEqual(rc, 0)
        self.assertFalse(mock_quotes.called)
        self.assertFalse(mock_attach.called)
        self.assertIn("默认跳过核心个股实时行情补厚", "\n".join(logs))

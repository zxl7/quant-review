...
class WorkflowScheduleTest(unittest.TestCase):
    def test_full_publish_never_writes_intraday_runtime_artifacts(self) -> None:
        ...
        self.assertIn("uses: ./.github/workflows/auction_prefetch.yml", intraday_workflow)
        self.assertIn("secrets: inherit", intraday_workflow)
        self.assertIn("scheduled_clock:", intraday_workflow)
        self.assertIn("wait_for_auction: true", intraday_workflow)
    ...
    def test_auction_workflow_isolated_from_full_publish(self) -> None:
        ...
        self.assertIn("wait_for_auction:", workflow)
        self.assertNotIn("schedule:", workflow)
        self.assertEqual(workflow.count('    - cron: "'), 0)
        self.assertIn("auction-prefetch-session", workflow)
        self.assertIn("cancel-in-progress: false", workflow)
        self.assertIn("timeout-minutes: 240", workflow)
        self.assertIn("actions: write", workflow)
        self.assertLess(workflow.index("- name: Resolve trade date"), workflow.index("- name: Wait until final cache refresh"))
        self.assertLess(workflow.index("- name: Restore auction cache"), workflow.index("- name: Wait until final cache refresh"))
        self.assertLess(workflow.index("- name: Check existing auction outcome"), workflow.index("- name: Wait until final cache refresh"))
        self.assertLess(workflow.index("- name: Wait until final cache refresh"), workflow.index("- name: Refresh published cache before capture"))
        self.assertIn("steps.existing.outputs.ready != 'true'", workflow)
        self.assertIn("inputs.wait_for_auction == true", workflow)
        self.assertIn("hour=9, minute=24, second=30", workflow)
    ...
    def test_execute_auction_snapshot_prefetch_skips_when_today_snapshot_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = execute_auction_snapshot_prefetch(cache_dir=cache_dir, trade_date10="2026-06-22")

        self.assertTrue(result["ok"])
        self.assertEqual(result["reason"], "snapshot_ready_skip")
        self.assertEqual(result["ready_source"], "prefetched_snapshot")

    def test_execute_auction_snapshot_prefetch_retries_once_then_succeeds(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            cache_dir = Path(tmp) / "cache"
            cache_dir.mkdir()
            ...
            with patch(
                ...
                side_effect=[
                    ...
                    ({"000001": {"dm": "000001", "t": "2026-06-22 09:25:03"}}, "2026-06-22 09:25:03"),
                ],
            ), patch(
                ...
                result = execute_auction_snapshot_prefetch(
                    cache_dir=cache_dir,
                    trade_date10="2026-06-22",
                    now=datetime(2026, 6, 22, 9, 25, 1, tzinfo=TZ_BJ),
                )
    ...
    def test_execute_auction_snapshot_prefetch_returns_failure_when_all_attempts_fail(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            cache_dir = Path(tmp) / "cache"
            cache_dir.mkdir()
            fake_rows = [{"date10": "2026-06-19", "code": "000001"}]
